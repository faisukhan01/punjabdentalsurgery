  ...
  return (
    <>
      ...
      <a
        ...
        className="fixed bottom-[92px] right-4 z-40 flex size-13 items-center justify-center rounded-full bg-red-700 text-white shadow-[0_10px_28px_rgb(156,28,35,0.45)] transition-transform hover:scale-105 active:scale-95 md:bottom-6 md:size-14"
      >
        <MessageCircle className="size-6 md:size-7" aria-hidden />
        <span className="absolute inset-0 -z-10 animate-ping rounded-full bg-red-700/40 [animation-duration:2.5s]" />
      </a>
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 border-t border-border/70 bg-white/90 p-3 backdrop-blur-md md:hidden [padding-bottom:calc(0.75rem+env(safe-area-inset-bottom))]">
        <div className="flex items-center gap-2.5">
          <Button
            className="h-11 flex-1 rounded-full text-[15px] font-semibold shadow-[0_8px_24px_rgb(156,28,35,0.35)]"
            onClick={() => openBooking()}
          >
